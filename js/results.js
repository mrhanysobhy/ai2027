...
function showResultsPage() {
    ...
    subjects.forEach(subject => {
        subject.lessons.forEach(lesson => {
            const active = getActiveLesson(subject.id, lesson.id);
            const saved = getStoredFinalResult(code, subject.id, lesson.id, active);
            if (saved) {
                results.push({
                    ...
                    total: saved.total,
                    passed: saved.passed,
                    attempt: saved.attempt || getStoredFinalAttempt(code, subject.id, lesson.id, active) || 1,
                    t: saved.t || 0,
            ...
            ${results.map(r => `
                <div class="result-item ${r.passed ? 'passed' : 'failed'}">
                    ...
                    <div class="result-details">
                        ...
                        <p>الدرجة: ${r.score} من ${r.total} | المحاولة: ${r.attempt} | التاريخ: ${new Date(r.t).toLocaleDateString('ar-EG')}</p>
...
function renderStudentProgress() {
    ...
    subjects.forEach(subject => {
        subject.lessons.forEach(lesson => {
            const active = getActiveLesson(subject.id, lesson.id);
            const saved = getStoredFinalResult(currentStudent.code, subject.id, lesson.id, active);
            if (saved) {
                completed++;
