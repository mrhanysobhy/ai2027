...
function admMatrixBody() {
  ...
  const list = STUDENT_LIST.filter((st) => !q || (st.name + ' ' + String(st.code)).toLowerCase().includes(q));
  ...
  const rowsHtml = list.map((st) => {
    const code = String(st.code);
    const cells = subjects.map((s) => {
      const mine = rows.filter((r) => String(r.studentCode) === code && r.subject === s.name);
      const passed = mine.some((r) => r.result === 'ناجح');
      const cls = passed ? 'ok' : (mine.length ? 'bad' : 'muted');
      const txt = passed ? '🏆 ناجح' : (mine.length ? '❌ راسب' : '—');
      return `<td><span class="tag ${cls}">${txt}</span></td>`;
    }).join('');
  ...
  return `
    ...
    <div class="pgr"><span class="chip mu">${list.length} طالب</span></div>`;
}
