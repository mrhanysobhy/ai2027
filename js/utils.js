...
// معرف الفترة للاختبار النهائي (يدخل في مفاتيح التخزين لتمييز فترة عن أخرى)
function getFinalExamId(lesson) {
    const exam = lesson && lesson.finalExam;
    if (exam && exam.examId) return exam.examId;
    if (exam && exam.availableFrom) return 'exam_' + String(exam.availableFrom).replace(/[^\w-]/g, '_');
    return 'exam_default';
}

function getFinalResultKey(code, subjectId, lessonId, lesson) {
    return `examResult_${code}_${subjectId}_${lessonId}_${getFinalExamId(lesson)}`;
}

function getFinalTakenKey(code, subjectId, lessonId, lesson) {
    return `examTaken_${code}_${subjectId}_${lessonId}_${getFinalExamId(lesson)}`;
}

function getFinalAttemptKey(code, subjectId, lessonId, lesson) {
    return `finalAttempt_${code}_${subjectId}_${lessonId}_${getFinalExamId(lesson)}`;
}

function getStoredFinalResult(code, subjectId, lessonId, lesson) {
    return JSON.parse(localStorage.getItem(getFinalResultKey(code, subjectId, lessonId, lesson)) || 'null');
}

function getStoredFinalAttempt(code, subjectId, lessonId, lesson) {
    return parseInt(localStorage.getItem(getFinalAttemptKey(code, subjectId, lessonId, lesson)) || '0', 10) || 0;
}
