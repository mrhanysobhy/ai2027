...
// اسم ورقة مواعيد الاختبارات (يُحررها المعلم مباشرة من الشيت)
const SCHEDULE_NAME = 'التوقيتات';
...
function onOpen() {
  ...
  ui.createMenu('AI Pioneers')
    ...
    .addItem('إنشاء/تهيئة ورقة المواعيد', 'setupScheduleSheet')
    .addSeparator()
...
function doPost(e) {
  ...
  // مزامنة مواعيد الاختبارات (من لوحة الإدارة) — تُنشئ/تعيد تعبئة ورقة التوقيتات
  if (payload.adminAction === 'syncSchedule') {
    if ((payload.password || '') !== ADMIN_PASSWORD) {
      return jsonResponse({ ok: false, error: 'كلمة مرور غير صحيحة' }, 403);
    }
    try {
      syncScheduleSheet_(payload.rows || []);
      return jsonResponse({ ok: true, count: (payload.rows || []).length });
    } catch (err) {
      return jsonResponse({ ok: false, error: 'تعذر حفظ المواعيد: ' + err.message }, 500);
    }
  }
 ...
 *
 * نقطة قراءة مواعيد الاختبارات (عامة — بلا كلمة مرور):
 * GET /exec?schedule=1 → { ok, count, rows } من ورقة التوقيتات
 */
function doGet(e) {
  if (e.parameter && e.parameter.schedule === '1') {
    return scheduleDoGet();
  }
...
function setupSheet() {
  ...
  // إنشاء ورقة المواعيد تلقائياً أولاً (دون مسح محتواها إن وُجدت)
  ensureScheduleSheet_();
  ...
  formatSheet_(sheet);
  SpreadsheetApp.getUi().alert('تم تهيئة ورقة النتائج + ورقة المواعيد بنجاح ✓');
}

/** إنشاء ورقة "التوقيتات" برؤوسها إن لم تكن موجودة (تحافظ على البيانات الحالية). */
function ensureScheduleSheet_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(SCHEDULE_NAME);
  if (!sheet) sheet = ss.insertSheet(SCHEDULE_NAME);
  if (sheet.getLastRow() === 0 && sheet.getMaxColumns() === 1) {
    sheet.getRange(1, 1, 1, SCHEDULE_HEADERS.length)
      .setValues([SCHEDULE_HEADERS])
      .setFontWeight('bold')
      .setBackground('#7a5a12')
      .setFontColor('#ffffff')
...
/**
 * قائمة > AI Pioneers > إنشاء/تهيئة ورقة المواعيد
 * يُنشئ ورقة "التوقيتات" برؤوسها (يحرر المعلم المواعيد منها مباشرة).
 */
function setupScheduleSheet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(SCHEDULE_NAME);
  if (!sheet) {
    sheet = ss.insertSheet(SCHEDULE_NAME);
  } else {
    const clear = SpreadsheetApp.getUi().alert(
      'الورقة موجودة',
      'هل تريد مسح محتواها وإعادة التهيئة؟',
  ...
  sheet.setFrozenRows(1);
  sheet.autoResizeColumns(1, SCHEDULE_HEADERS.length);
  SpreadsheetApp.getUi().alert('تم إنشاء ورقة المواعيد ✓ — أضف صفاً لكل درس (كود المادة، رقم الدرس، البداية، النهاية).');
}

/** قراءة صفوف ورقة المواعيد وعرضها كـ JSON عام (للطلاب عند الدخول). */
function scheduleDoGet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SCHEDULE_NAME);
  const rows = [];
  if (sheet && sheet.getLastRow() > 1) {
    const data = sheet.getRange(2, 1, sheet.getLastRow() - 1, SCHEDULE_HEADERS.length).getValues();
    const tz = Session.getScriptTimeZone();
    for (let i = 0; i < data.length; i++) {
      const r = data[i];
      rows.push({
        subjectId: String(r[0] || '').trim(),
        subjectName: String(r[1] || ''),
        lessonNo: Number(r[2]) || 0,
        lessonTitle: String(r[3] || ''),
        examId: String(r[4] || '').trim(),
        availableFrom: cellToIso_(r[5], tz),
        availableTo: cellToIso_(r[6], tz)
      });
    }
  }
  return jsonResponse({ ok: true, count: rows.length, rows: rows });
...
/** منسق الخلية إلى صيغة ISO نصية (يدعم خلية تاريخ/وقت حقيقية أو نصاً). */
function cellToIso_(v, tz) {
  if (v instanceof Date) return Utilities.formatDate(v, tz, 'yyyy-MM-dd HH:mm');
  return String(v || '').trim();
}

/** كتابة كل صفوف المواعيد (تُستدعى من doPost لزر "مزامنة المواعيد"، ويمحو ما قبله). */
function syncScheduleSheet_(rows) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(SCHEDULE_NAME);
  if (!sheet) sheet = ss.insertSheet(SCHEDULE_NAME);
  sheet.clear();
  ...
  if (rows && rows.length) {
    ...
    const out = rows.map((r) => {
      const cell = (v) => (v instanceof Date ? Utilities.formatDate(v, tz, 'yyyy-MM-dd HH:mm') : String(v || '').trim());
      return [
        String(r.subjectId || '').trim(),
        String(r.subjectName || ''),
        Number(r.lessonNo) || 0,
        String(r.lessonTitle || ''),
        String(r.examId || '').trim(),
        cell(r.availableFrom),
        cell(r.availableTo)
      ];
    });
    sheet.getRange(2, 1, out.length, SCHEDULE_HEADERS.length).setValues(out);
  }
  sheet.autoResizeColumns(1, SCHEDULE_HEADERS.length);
}
