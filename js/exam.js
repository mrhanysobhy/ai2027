...
// نافذة الاختبار من الإعداد الفعلي (مواعيد الشيت تسبق المدمجة بالدرس)
function finalWindow(sbj, n, lesson) {
  const ex = effectiveFinal(sbj, n, lesson);
  const now = Date.now();
...
function parseFinalTime(value, isEnd) {
  ...
  const iso = v.replace(' ', 'T'); // يدعم صيغة الشيت "yyyy-MM-dd HH:mm"
...
function doneEx(auto) {
  ...
  // نهائي
  const passScore = (effectiveFinal(x.sbj.id, x.n, x.lesson).passScore || 60);
