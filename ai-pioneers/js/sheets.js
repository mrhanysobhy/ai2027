...
// قراءة المواعيد مرة واحدة (تُستدعى عند دخول الطالب) — بلا كلمة مرور
async function fetchSchedule() {
  ...
  try {
    const sep = SHEET_URL.includes('?') ? '&' : '?';
    const res = await fetch(SHEET_URL + sep + 'schedule=1', { signal: ctrl.signal });
    if (!res.ok) return { ok: false, error: 'فشل جلب المواعيد (' + res.status + ')' };
    const data = await res.json();
    return { ok: !!data.ok, rows: data.rows || [], count: data.count || 0, error: data.error };
  } catch (e) {
    return { ok: false, error: e.name === 'AbortError' ? 'انتهت مهلة جلب المواعيد' : 'تعذّر جلب المواعيد' };
  } finally {
    clearTimeout(timer);
  }
}

// مزامنة مواعيد الدروس الحالية إلى ورقة "التوقيتات" (من لوحة الإدارة)
async function pushSchedule(rows) {
  const IS_DEV = /^(localhost|127\.0\.0\.1)$/.test(String(location.hostname));
  if (IS_DEV) {
    console.log('[dev] تم تخطي مزامنة المواعيد لأن التطبيق يعمل محلياً (localhost).');
    return { ok: false, error: 'وضع التطوير (localhost) يمنع المزامنة' };
  }
  if (!SHEET_URL || SHEET_URL === 'GOOGLE_APPS_SCRIPT_URL_HERE') {
    return { ok: false, error: 'رابط Google Sheets غير مُعيّن' };
  }
  const adminPw = (typeof ADMIN_PW !== 'undefined') ? ADMIN_PW : '';
  try {
    await fetch(SHEET_URL, {
      method: 'POST',
      mode: 'no-cors',
      headers: { 'Content-Type': 'text/plain' },
      body: JSON.stringify({ adminAction: 'syncSchedule', password: adminPw, rows: rows || [] })
    });
    console.log('تم إرسال المواعيد إلى Google Sheets:', (rows || []).length, 'صف');
    return { ok: true };
  } catch (e) {
    console.error('فشل مزامنة المواعيد:', e);
    return { ok: false, error: 'فشل الاتصال بالشيت' };
  }
}