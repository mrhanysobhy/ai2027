...
function fdt(value) {
  if (!value) return '';
  const hasTime = /T/.test(String(value)) || /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}/.test(String(value));
  let s = value;
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) s += 'T00:00:00';
  else s = String(s).replace(' ', 'T');
  const d = new Date(s);
...
/* ────────────────────────── مواعيد الاختبارات (ورقة "التوقيتات") ────────────────────────── */

let SCHED = null;          // صفوف مواعيد الشيت بعد التحميل
let SCHED_LOADED = false;  // هل انتهى محاولة التحميل (نجحت أم فشلت)
const SCHED_CACHE_KEY = 'scheduleCache';
const SCHED_MAX_AGE = 15 * 60 * 1000; // كاش 15 دقيقة

// تحميل المواعيد مرة واحدة عند دخول الطالب (يتملى من الكاش إن كان حديثاً)
async function loadSchedule() {
  try {
    const c = JSON.parse(ls.g(SCHED_CACHE_KEY) || 'null');
    if (c && Array.isArray(c.rows) && (Date.now() - (c.t || 0)) < SCHED_MAX_AGE) {
      SCHED = c.rows;
      SCHED_LOADED = true;
      applyScheduleUi();
      return;
  ...
  if (res.ok && Array.isArray(res.rows)) {
    SCHED = res.rows;
    ls.s(SCHED_CACHE_KEY, JSON.stringify({ t: Date.now(), rows: res.rows }));
  } else {
    SCHED = null;
  }
  SCHED_LOADED = true;
...
// صف التوقيتات المطابق لمادة+درس (رقم الدرس) أو null
function schedRow(sbj, n) {
  if (!SCHED || !Array.isArray(SCHED) || !sbj) return null;
  return SCHED.find((r) => r && String(r.subjectId) === String(sbj.id) && Number(r.lessonNo) === Number(n)) || null;
}

// إعداد الاختبار النهائي الفعلي: مواعيد الشيت أولاً (إن وجدت) ثم المدمجة بالدرس خلفاً
// ملاحظة: examId يبقى من درس JSON (هو معرف الفترة المعتمد في مفاتيح التخزين)؛
// عمود "معرف الاختبار" في ورقة المواعيد مرجعي فقط لأرشفة الفترات.
function effectiveFinal(sbj, n, lesson) {
  const base = (lesson && lesson.finalExam) || {};
  const r = schedRow(sbj, n);
  if (!r) return { ...base };
  return {
    ...base,
    availableFrom: r.availableFrom || base.availableFrom,
    availableTo: r.availableTo || base.availableTo
  };
}
...
// هل يشير المفتاح لدرس/فترة غير سارية الآن؟
function isStaleExamKey(key) {
  if (typeof localStorage === 'undefined') return false;
  const m = String(key).match(/^(examResult|examTaken|finalAttempt)_(\d+)_([^_]+)_(\d+)(?:_(.+))?$/);
  if (!m) return false;
  const code = m[2], sid = m[3], lid = m[4], suffix = m[5];
  if (!(code in ST)) return true;                    // كود غير موجود بقائمة الطلاب
  const sbj = sub(sid);
  const lesson = lessonContent(sbj, Number(lid));
  if (!lesson || !lesson.finalExam) return true;      // مادة/درس غير موجودة حالياً
  if (suffix === undefined) return true;              // مفتاح قديم بلا معرف فترة
  return suffix !== examIdOf(lesson);                 // فترة سابقة غير سارية
}

// تنظيف مفاتيح الأداء القديمة من النسخ السابقة — تُستدعى عند فتح الموقع
// التعامل مع الفترة: مفتاح ذو examId غير سارٍ حالياً يُحذف (كسر "نتائج سابقة تحجب فترة جديدة").
// نتائج الفترة الحالية (examId مطابق) تُحفظ — لا تمسح النجاحات الصالحة.
function sanitizeLocalStorage() {
  if (typeof localStorage === 'undefined') return 0;
  const build = window.APP_BUILD || 'dev';
  const prev = ls.g('appBuild');
  const keys = [];
  for (let k = 0; k < localStorage.length; k++) {
    const key = localStorage.key(k);
    if (!key) continue;
    if (/^(examResult|examTaken|finalAttempt)_\d+_/.test(key)) keys.push(key);
  }

  keys.forEach((key) => { if (isStaleExamKey(key)) ls.d(key); });

  // تجديد بسيط عند تغيّر البناء: يُلغى كاش المواعيد فقط (ردة مواعيد قديمة)؛
  // لا تُمسح نتائج الفترة الحالية حتى لا تنكسر الشهادة/الاجتياز.
  if (prev !== build) {
    ls.d(SCHED_CACHE_KEY);
    ls.s('appBuild', build);
  }
  return keys.length;
}